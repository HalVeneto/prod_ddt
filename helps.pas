...
begin
  ...
  if trim(Ecerca.Text)<>'' then begin
    zqh.Filtered:=false;
    if not zqH.Locate(zqh.Fields[0].FieldName,Ecerca.Text,[loCaseInsensitive,loPartialKey]) then
...
begin
  ...
  if Fel.Left+Fel.Width>screen.Width then //se estremità dx esce da screen, sposta a sx
    Fel.Left:=10;
  //--->ampiezza minima 520 messa nel constraint<---
  w:=0;
...
procedure elenca(sql:widestring; ccod:byte; tit:string; head:array of string); //ccod: numero campi codice da ritornare (da porre all'inizio elenco; non sono visualizzati nell'help)
var
  p,psz,x:byte;
  w,wdt:word;
  titolo:string;
...
begin
  with Fel do begin
    ...
    //nasconde campi codice nella finestra:
